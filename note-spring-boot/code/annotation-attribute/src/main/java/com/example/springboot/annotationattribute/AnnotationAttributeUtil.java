...
public class AnnotationAttributeUtil {
    ...
    private static Map<String, Object> getMergedAnnotationAttributes(AnnotatedElement annotatedElement,
        ...
        for (Annotation annotation : annotations) {
            ...
            if (annotationType.equals(annType)) {
                return getMergedAttributes(new AnnotationHolder(annotation, annotatedAnnotationHolder));
            }
        }
    ...
    private static Map<String, Object> getMergedAttributes(AnnotationHolder annotationHolder) {
        ...
        for (Method attribute : attributes) {
            ...
            annotationAttributes.put(attributeName, getMergedAttributeValue(annotationHolder, attributeName));
        }
        return annotationAttributes;
    }

    private static Object getMergedAttributeValue(AnnotationHolder annotationHolder, String attributeName) {
        ...
        if (NULL_ATTRIBUTE_VALUE.equals(attributeValue)) {
            attributeValue = getMergedValueFromAttributeOrAliases(annotationHolder.self, attributeName);
        }
    ...
    private static Object overrideAttributeValue(AnnotationHolder childAnnotationHold,
        ...
        return getMergedValueFromAttributeOrAliases(childAnnotationHold.self, attributeName);
    }
    ...
    /**
     * 1) Explicit Aliases: if two attributes in one annotation are declared as aliases for each other via @AliasFor,
     *    they are explicit aliases.
     * 2) Implicit Aliases: if two or more attributes in one annotation are declared as explicit overrides for the same
     *    attribute in a meta-annotation via @AliasFor, they are implicit aliases.
     * 3) Transitive Implicit Aliases: given two or more attributes in one annotation that are declared as explicit
     *    overrides for attributes in meta-annotations via @AliasFor, if the attributes effectively override the same
     *    attribute in a meta-annotation following the law of transitivity, they are transitive implicit aliases.
     */
    private static Object getMergedValueFromAttributeOrAliases(Annotation annotation, String attributeName) {
        Class<? extends Annotation> annotationType = annotation.annotationType();
        Method[] attributes = annotationType.getDeclaredMethods();
        if(attributes.length > 0) {
            Method attribute = getAttribute(attributes, attributeName);
            if(attribute != null) { // there is attribute with supplied 'attributeName'
                AliasForWrapper aliasForWrapper = AliasForWrapper.build(attribute);
...
//                Object attributeValue = getMergedValueFromExplicitAlias();
//                if (NULL_ATTRIBUTE_VALUE.equals(attributeValue)) {
//
//                }
            }
